        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            comboMenu.AddItem(
                new MenuItem("ComboMode", "Combo Mode:").SetValue(
                        new StringList(new[] {"Burst", "OldBurst", "Survivor Mode (Shield)", "As Fast As Possible (Spam)"}))
...
#pragma warning restore CS0618 // Type or member is obsolete
                ...
                case 0:
                    ...
                    if (Player.Mana >= _q.Instance.ManaCost + _w.Instance.ManaCost + _e.Instance.ManaCost)
                    {
                        if (cUseQ && target.IsValidTarget(_q.Range) && _q.IsReady())
                            SebbySpell(_q, target);
                        if (cUseE && target.IsValidTarget(_e.Range) && _e.IsReady() && !_q.IsReady())
                            _e.CastOnUnit(target);
                        if (cUseW && target.IsValidTarget(_w.Range) && _w.IsReady() && !_q.IsReady() && !_e.IsReady())
                            _w.CastOnUnit(target);
        ...
        private static void LastHit()
        {
            ...
            if (Player.ManaPercent > _menu.Item("LaneHitManaManager").GetValue<Slider>().Value)
            {
                ...
                var allMinionsAutoAttack = Cache.GetMinions(Player.Position, 550);
                if (_q.IsReady() && useQ)
                {
                    if (allMinionsQ.Count > 0)
                        foreach (var minion in allMinionsQ)
                        {
                            if (!minion.IsValidTarget() || (minion == null))
                                return;
                            if (minion.Health +5 < _q.GetDamage(minion))
                                _q.Cast(minion);
                        }
                }
                if (_e.IsReady() && useE )
                {
                    if (allMinionsE.Count > 2)
                        foreach (var minion in allMinionsE)
                        {
                            if (!minion.IsValidTarget() || (minion == null))
                                return;
                            if (minion.Health +5 < _e.GetDamage(minion))
        ...
        {
            float damage = 0;
            if (_q.IsReady() || (Player.Mana <= _q.Instance.ManaCost + _q.Instance.ManaCost))
                damage += QGetRealDamage(enemy) + QGetRealDamage(enemy);
            else if (_q.IsReady() || (Player.Mana <= _q.Instance.ManaCost))
                damage += QDefaultDamage(enemy);

            if (_w.IsReady() || (Player.Mana <= _w.Instance.ManaCost))
                damage +=  _w.GetDamage(enemy);
            else if (_w.IsReady() || (Player.Mana <= _w.Instance.ManaCost))
                damage += _w.GetDamage(enemy);

            if (_e.IsReady() || (Player.Mana <= _e.Instance.ManaCost ))
